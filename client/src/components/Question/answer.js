...
const checkAnswer = (db, sql, question) => {
  ...
  if (question.keywords) {
    ...
    for (let i = 0; i < keywords.length; i++) {
      ...
      lastIndex = sqlUppercase.indexOf(keywords[i].toUpperCase(), lastIndex + 1);

      // If the keyword was not found
      if (lastIndex === -1) {
        throw new IncorrectAnswer(`Looking for the incursion of the keyword: ${keywords[i]}, but not found or found in the wrong position!`);
      }
    }
  }

  const [{ columns: userColumns, values: userValues }] = db.exec(sql);
  const [{ columns: modelColumns, values: modelValues }] = db.exec(question.answer);
  ...
  if (modelColumns.length !== userColumns.length) {
    throw new IncorrectAnswer(`Expected a total of ${modelColumns.length} column(s) to be returned, instead got ${userColumns.length}!`);
  }

  // 3. Check both result sets are of equal length
  if (modelValues.length !== userValues.length) {
    throw new IncorrectAnswer(`Expected a total of ${modelValues.length} row(s) to be returned, instead got ${userValues.length}!`);
  }
  ...
  normalizedUserColumns.forEach((column, userColumnIndex) => {
    ...
    const modelColumnValues = modelValues.map(result => result[modelColumnIndex]);
    ...
    modelColumnValues.forEach(value => {
      ...
      if(foundIndex === -1) {
        throw new IncorrectAnswer(`The column value ${value} was not found in the column ${modelColumns[modelColumnIndex]}`);
      }
